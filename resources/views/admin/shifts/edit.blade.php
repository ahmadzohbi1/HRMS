            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('shifts.update', $shift->id) }}">
                        ...
                        <div id="department_field" class="form-group mb-3"
                            ...
                            <select id="department_id" name="department_id" class="form-control">
                                @foreach($departments as $department)
                                    <option value="{{ $department->id }}" {{ $shift->department_id == $department->id ? 'selected' : '' }}>{{ $department->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div id="employee_field" class="form-group mb-3"
                            ...
                            <select id="employee_id" name="employee_id" class="form-control">
                                @foreach($employees as $employee)
                                    <option value="{{ $employee->id }}" {{ $shift->employee_id == $employee->id ? 'selected' : '' }}>{{ $employee->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <!-- Shift Rules -->
                        <div class="form-group mb-3">
                            <label for="shift_rules">Shift Rules</label>
                            <div>
                                @foreach($shiftRules as $rule)
                                    <div class="form-check">
                                        <input type="checkbox" class="form-check-input" id="rule_{{ $rule->id }}"
                                            name="shift_rules[]" value="{{ $rule->id }}" {{ $shift->shiftRules->contains($rule->id) ? 'checked' : '' }}>
