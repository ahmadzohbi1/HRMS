                ...
                <div class="card-body">
                    <form action="{{ route('vacations.update', $vacation->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="employee_id" name="employee_id" required>
                                @foreach($employees as $employee)
                                    <option value="{{ $employee->id }}" {{ $vacation->employee_id == $employee->id ? 'selected' : '' }}>{{ $employee->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="vacation_type_id" name="vacation_type_id" required>
                                @foreach($vacationTypes as $type)
                                    <option value="{{ $type->id }}" {{ $vacation->vacation_type_id == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            <label for="start_date" class="form-label">Start Date</label>
                            <input type="date" class="form-control" id="start_date" name="start_date" value="{{ $vacation->start_date }}" required>
                        </div>
                        <div class="mb-3">
                            <label for="end_date" class="form-label">End Date</label>
                            <input type="date" class="form-control" id="end_date" name="end_date" value="{{ $vacation->end_date }}" required>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="reason" name="reason" rows="3" required>{{ $vacation->reason }}</textarea>
                        </div>
